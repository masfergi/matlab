...
%O=[1 2;-2+j 1+j];                                  % Alamouti Scheme [x1 x2;-x2* x1*] 
O=[1 -2 -3;2+j 1+j 0;3+j 0 1+j;0 -3+j 2+j];         % for [x1 -x2 -x3;x2* x1* 0;x3* 0 x1*;0 -x3* x2*] 
co_time=size(O,1);                                                                  
Nt=size(O,2);                                                                        
Nr=2;                                                                              
...
carriers = (1: carrier_count) + (floor(IFFT_bin_length/4) - floor(carrier_count/2));   % ×ø±ê£º (1 to 100) + 14=(15:114)
conjugate_carriers=IFFT_bin_length-carriers+2;                                         % ×ø±ê £º256 - (15:114) + 1= 257 - (15:114) = (242:143) 
